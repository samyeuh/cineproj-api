...
public class UserDAO {
	...
	public void updateUser(User user) throws SQLException {
	    ...
	    if (password == null) {
	        // aller récupérer l'ancien password en base
	        PreparedStatement selectStmt = conn.prepareStatement("SELECT password FROM users WHERE id = ?");
	        selectStmt.setObject(1, user.getId());
	        ResultSet rs = selectStmt.executeQuery();
	        if (rs.next()) {
	            password = rs.getString("password");  // on récupère l'ancien password
	        }
	    }
